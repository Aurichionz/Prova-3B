  ...
  return (
    <>
      ...
      {permissions[props.crud[CREATE]] && (
        <ButtonAtualizar className="me-1" onClick={() => navigate('create')}>
      ...
      <tr>
        ...
        <tbody>
          {props.data.map((element, rowIndex) => (
            <tr key={rowIndex}>
              {props.keys.map((key, colIndex) => (
                props.hide[colIndex] 
                ? <td className='d-none d-md-table-cell' key={colIndex}>{element[key]}</td>
                : <td key={colIndex}>{element[key]}</td>
              ))}
              <td className={props.hide[props.keys.length] ? 'd-none d-md-table-cell' : ''}>
                {permissions[props.crud[EDIT]] && (
                  <ButtonAtualizar className="me-1" onClick={() => update(element)}>
                ...
                {permissions[props.crud[DELETE]] && (
                  <ButtonRemover className="me-1" onClick={() => remove(element)}>
      ...
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
        ...
        <Modal.Footer>
          <ButtonAtualizar onClick={handleClose}>OK</ButtonAtualizar>
        </Modal.Footer>
      </Modal>
    </>
  );
